...
| **Date** | {DATE} |
| **Commit** | {COMMIT_SHA} |
| **Auditor** | /codebase-audit v1.0.0 |
| **Runtime** | {RUNTIME} |
| **Framework** | {FRAMEWORK} |
| **LOC** | {LOC} |
| **Files** | {FILE_COUNT} |
| **Test Files** | {TEST_FILE_COUNT} |
| **Mode** | {MODE} (may include `--suggest-fixes`, `--quick-fix`, `--changed-only`, `--ci`, `--json`, `--min-severity`) |
| **Duration** | {DURATION} |
...
**Suggested Fix:** {CONFIDENCE_TAG}
```diff
{DIFF}
```
{DIFF_NOTE — e.g., "Apply same pattern to N other locations" or "Fix upstream in [package]"}
